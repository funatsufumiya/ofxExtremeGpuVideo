...
//        }
        static std::map<std::string, bool> hasset_map;
        if(hasset_map.find(name) == hasset_map.end()){
            hasset_map[name] = true;
...
void ofApp::draw() {
    ...
    if(!once_flag){
        once_flag = true;
        ImGui::SetNextWindowPos(ofVec2f(10, 30));
        ImGui::SetNextWindowSize(ofVec2f(ofGetWidth() - 50, ofGetHeight() - 50));
    }
